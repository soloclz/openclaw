...
| `configSchema`                      | Yes      | `object`                         | Inline JSON Schema for this plugin's config at `plugins.entries.<id>.config`.                                                                                                                                |
| `providerConfigSchema`              | No       | `object`                         | Inline JSON Schema for this plugin's provider config at `models.providers.<providerId>`. Required when a plugin owns a model provider and that provider's config is not already covered by the core schema.  |
